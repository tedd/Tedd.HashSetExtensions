...
2. CI pipeline `azure-pipelines.yml` does not explicitly manage SDK versions for multi-targeting, needing `.NET 10.0.x` and `8.0.x` to reliably test targets. The `workingDirectory` flag along with `projects` defaults caused `dotnet restore` to fail when running from `src`.
...
2. Updated `azure-pipelines.yml` to use `UseDotNet@2` for 10.0.x (with includePreviewVersions: true) and 8.0.x, and removed conflicting `workingDirectory` inputs when using a root wildcard (`src/**/*.csproj`).
